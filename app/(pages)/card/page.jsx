...
const CATEGORY_STYLES = {
  ...
  anime: {
    ...
    iconSolid: (
      <Wand2 className="h-5 w-5 text-emerald-400 fill-emerald-400/20" />
    ),
  ...
  books: {
    ...
    iconSolid: (
      <Bookmark className="h-5 w-5 text-amber-600 fill-amber-600/20" />
    ),
  ...
  songs: {
    ...
    iconSolid: (
      <Headphones className="h-5 w-5 text-purple-400 fill-purple-400/20" />
    ),
...
export default function IceCardMaker() {
  ...
  useEffect(() => {
    ...
    const topCategories = Object.entries(contentData)
      .filter(([key]) => Object.keys(CATEGORY_STYLES).includes(key))
      .sort(
        (a, b) => (b[1].completed?.length || 0) - (a[1].completed?.length || 0)
      );
    ...
    setCustomDescription(
      `✨ ${username}'s ${topCategories
        .slice(0, 3)
        .map(([key]) => CATEGORY_STYLES[key]?.name || "")
        .filter(Boolean)
        .join(" • ")} journey`
    );
  }, [contentData, user]);
  ...
  const calculateStats = () => {
    ...
    const result = Object.keys(CATEGORY_STYLES).reduce(
      (acc, category) => {
        ...
        if (
          !acc.topCategory ||
          acc.byCategory[category].completed >
            acc.byCategory[acc.topCategory].completed
        ) {
  ...
  const profileUrl = `https://icebreaker-nine.vercel.app/profile/${
    user?.id || "user"
  }`;
  const topCategoryData = stats.topCategory
    ? CATEGORY_STYLES[stats.topCategory]
    : null;
        ...
        <div className="flex-1">
          ...
          >
            <div className="p-6 text-white">
              ...
              <div className="flex items-center justify-between mb-4">
                  ...
                  <div>
                    ...
                    {topCategoryData && (
                      <p className="text-sm flex items-center gap-1 mt-1">
                        <span
                          className={`px-2 py-0.5 rounded-full ${topCategoryData.bg} border ${topCategoryData.border} text-xs`}
                        >
                    ...
                    <Link href={`${profileUrl}`}>
                      {" "}
                      <Share2
                        className={`h-4 w-4 ${CATEGORY_STYLES[selectedTheme].accent} mt-1`}
                      />
                    </Link>
                  </div>
                </div>
                ...
                >
                  <QRCode
                    ...
                    size={qrCodeSize}
                    level="H"
                    fgColor={CATEGORY_STYLES[selectedTheme].border.replace(
                      "border-",
                      ""
                    )}
                    bgColor="transparent"
              ...
              {topCategoryData && (
                <div
                  className={`mb-6 p-4 rounded-lg ${topCategoryData.bg} border ${topCategoryData.border}`}
                >
                  <div className="flex items-center gap-2 mb-2">
                    {topCategoryData.icon}
                    <h3 className="font-medium">
                      {topCategoryData.name} Enthusiast
                    </h3>
                    <span className="text-sm ml-auto bg-white/10 px-2 py-0.5 rounded">
                      {stats.byCategory[stats.topCategory]?.completed || 0}{" "}
                      completed
                    </span>
                  </div>
                  {editMode ? (
              ...
              {/* Footer CTA */}
              <div
                className={`p-4 text-center rounded-lg ${CATEGORY_STYLES[selectedTheme].bg} border ${CATEGORY_STYLES[selectedTheme].border}`}
              >
                <h4 className="font-medium text-xl mb-2">
                  {CATEGORY_STYLES[selectedTheme].cta}
                </h4>
        ...
        <div className="lg:w-80 space-y-6">
          <div className="bg-gray-900/50 border-0 border-gray-700 rounded-xl p-6">
            ...
            <div className="space-y-5">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {Object.entries(CATEGORY_STYLES).map(([key, theme]) => (
                    <button
                      ...
                      className={`p-3 rounded-lg border-2 text-purple-300 hover:text-white transition-all ${
                        selectedTheme === key
                    ...
                    >
                      <div className="flex flex-col items-center gap-2">
                        <div
                          className={`p-2 rounded-full ${theme.bg} border ${theme.border}`}
                        >
                          {theme.icon}
                        </div>
              ...
              <div>
                <label className="block text-md font-medium text-blue-300 mb-2">
                  Description
                </label>
                <Textarea
                  value={customDescription}
